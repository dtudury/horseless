...
export function renderAttributes (attributes, node) {
  let obj = {}
  attributes.forEach(attribute => {
    ...
    if (attribute && attribute.type === 'attribute') {
      ...
      obj = Object.assign(renderAttributes(attribute, node), obj)
        ...
        if (description.tag === null || description.tag === '') {
          ...
          if (!_descriptionMap.has(description)) {
            ...
            if (description.type === 'textnode') {
              ...
              let oldAttributes = {}
              let newAttributes = renderAttributes(description.attributes, node)
              ...
              watchFunction(() => {
                newAttributes = renderAttributes(description.attributes, node)
                _setAttributes(node, newAttributes)
