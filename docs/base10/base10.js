...
function box (x, y, z, width, height, depth, scale, shadow) {
  ...
  let _shadow = []
  if (shadow === 'bottom') {
    _shadow = h`<path d=${bottom} stroke="none" fill="#aa8866cc" filter="url(#shadow)"/>`
  } else if (shadow === 'all') {
    _shadow = h`<path d=${all} stroke="none" fill="#aa8866cc" filter="url(#shadow)"/>`
  }
  return h`
    ${_shadow}
...
function boxes (el) {
  ...
  const depth = 1
  const scale = 20
  return [...Array(9).keys()].map(i => box(x + i * 1.3, y - i * 0, 0 * i, width, height, depth, scale, i === 0 ? 'bottom' : 'all'))
}

render(document.querySelector('main.card'), h`
  <svg width="100%" height="100%" viewBox="0 0 600 400" fill="none" stroke="#aa8866" stroke-linejoin="round" stroke-linecap="round" stroke-width="1" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <filter id="shadow" x="-50%" y="0%" width="200%" height="200%">
        <feOffset in="SourceGraphic" dy="5" />
